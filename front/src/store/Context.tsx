...
interface SourceContextValue {
  ...
  updateData: (newSources: Array<{ type: Source; userInput: string }>, newUserCountry: string) => void;
}
...
export const SourceProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [sources, setSources] = useState<Array<{ type: Source; userInput: string }>>([]);
  const [userCountry, setUserCountry] = useState("");
  const [time,setTime]=useState('');

  const updateData = (newSources: Array<{ type: Source; userInput: string }>, newUserCountry: string) => {
  ...
  return (
    <SourceContext.Provider value={{ sources, userCountry, updateData,updateTime,time }}>
      {children}
